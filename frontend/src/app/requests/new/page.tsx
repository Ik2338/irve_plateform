...
import {
  ...
  Hotel, Users, MapPin, Plug, ParkingSquare, AlertCircle,
} from 'lucide-react';
...
import {
  ...
  POWER_LEVEL_LABELS,
  POWER_LEVEL_DESCRIPTIONS,
  POWER_LEVEL_SHORT,
  CONNECTOR_LABELS,
  PARKING_TYPE_LABELS,
  PARKING_ACCESS_LABELS,
  type ProjectType,
  type PowerLevel,
  type ConnectorType,
  type ParkingType,
...
interface FormData {
  ...
  address: string;
  city: string;
  postalCode: string; // ✅ renamed from zipCode to match backend DTO
...
export default function NewRequestPage() {
  ...
  const canNext = () => {
    ...
    if (step === 3) return (
      !!form.address.trim() &&
      !!form.city.trim() &&
      !!form.postalCode.trim() && // ✅ validates postalCode (was zipCode, and was missing validation)
      !!form.parkingType &&
      !!form.parkingAccess
    );
    return true;
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      // ✅ Only send fields that exist in CreateRequestDto.
      // Removed: connectors, parkingType, parkingAccess, parkingSpots (not in backend DTO).
      // Fixed: postalCode (was zipCode — mismatch was causing the 400 error).
      await requestsApi.create({
        ...
        city:             form.city,
        postalCode:       form.postalCode, // ✅ was "zipCode" — backend expects "postalCode"
      ...
      router.push('/dashboard');
        ...
        console.error('DETAIL:', err.response?.data); // ← ajoutez ça
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">Décrivez votre projet, nous trouvons l'installateur certifié le plus proche.</p>
        </div>
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                {POWER_LEVELS.map(level => {
                  ...
                  return (
                    <button
                      key={level}
                      onClick={() => setForm(f => ({ ...f, powerLevel: level }))}
            ...
            <div>
                ...
                {CONNECTORS.map(c => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-sm font-medium flex-1 ${selected ? 'text-primary' : 'text-gray-700'}`}>
                        {CONNECTOR_LABELS[c]}
                      </span>
              ...
              {form.connectors.length === 0 && (
                <p className="text-xs text-red-500 mt-2">Sélectionnez au moins un connecteur.</p>
              )}
            </div>
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mb-3">Combien de bornes souhaitez-vous installer ?</p>
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setForm(f => ({ ...f, quantity: Math.max(1, f.quantity - 1) }))}
                  className="w-10 h-10 rounded-xl border-2 border-gray-200 flex items-center justify-center font-bold text-gray-600 hover:border-primary hover:text-primary transition-all"
                >−</button>
                <div className="w-20 h-10 rounded-xl border-2 border-primary bg-primary-light flex items-center justify-center font-bold text-primary text-lg">
                  {form.quantity}
                </div>
                <button
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {PARKING_TYPES.map(pt => (
                      <button
                        key={pt}
                        onClick={() => setForm(f => ({ ...f, parkingType: pt }))}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {PARKING_ACCESS.map(pa => (
                      <button
                        key={pa}
                        onClick={() => setForm(f => ({ ...f, parkingAccess: pa }))}
                        className={`text-left px-4 py-3 rounded-xl border-2 text-sm font-medium transition-all
                          ${form.parkingAccess === pa
                            ? 'border-primary bg-primary-light text-primary'
                            : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300'}`}
                      >
                ...
                {/* Tableau existant */}
                <button
                  onClick={() => setForm(f => ({ ...f, hasExistingPanel: !f.hasExistingPanel }))}
                  className={`w-full text-left p-3.5 rounded-xl border-2 flex items-center gap-3 transition-all
                    ${form.hasExistingPanel
                      ? 'border-primary bg-primary-light'
                      : 'border-gray-200 bg-white hover:border-gray-300'}`}
                >
                  <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0
            ...
            <div className="card">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                {[
                  ...
                  { label: 'Adresse',           value: `${form.address}, ${form.postalCode} ${form.city}` }, // ✅ was zipCode
