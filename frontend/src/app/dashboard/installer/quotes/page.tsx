...
import { Plus, FileText, Clock, CheckCircle, Trash2, AlertTriangle, Shield, User } from 'lucide-react';
...
// ── Bandeau "envoyée à [installateur]" ──────────────────────────────────────
function InstallerBadge({ installer }: { installer: any }) {
  if (!installer) return null;
  return (
    <div className="flex items-center gap-1.5 mt-1.5">
      <Shield className="w-3 h-3 text-green-600 flex-shrink-0" />
      <span className="text-xs text-green-700 font-medium">
        Envoyée à <span className="font-semibold">{installer.companyName}</span>
        ...
        {requests.length === 0 ? (
          ...
          <div className="space-y-3 mb-8">
            {requests.map(r => {
              ...
              // L'installateur ciblé peut être dans r.installer ou r.targetInstaller selon votre API
              const targetInstaller = r.installer || r.targetInstaller || null;
                ...
                <div key={r.id} className="card">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {/* ✅ Badge installateur ciblé */}
                      <InstallerBadge installer={targetInstaller} />
                    ...
                    {/* Droite : badge statut + suppression */}
